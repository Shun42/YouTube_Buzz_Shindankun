...
export async function createStrongWordsScatter() {
  ...
  const strong_words_scatter_Container = document.getElementById("strong_words_scatter_videos")

  if (!strong_words_scatter_Container) return;
  ...
  const strong_words_scatter_Html = strong_words_scatter_data.map((strong_words_scatter_Info) => {
    return `
      <tr>
        <td>${strong_words_scatter_Info.title}</td>
        <td>
          <a href="https://www.youtube.com/watch?v=${strong_words_scatter_Info.video_id}" target="_blank" rel="noopener noreferrer">
            https://www.youtube.com/watch?v=${strong_words_scatter_Info.video_id}
          </a>
        </td>
        <td>${strong_words_scatter_Info.buzz_type}</td>
      </tr>
    `;
  }).join("");
  strong_words_scatter_Container.innerHTML = `
    <table>
      <thead>
        <tr>
          <th>動画タイトル</th>
