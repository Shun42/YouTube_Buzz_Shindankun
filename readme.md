...
## 1. プロジェクト概要
Youtube Buzz Analyzer v2は、YouTube動画のタイトル、タグ、コメント、字幕、再生数、高評価数などのデータを取得し、再生数が伸びている動画の特徴を分析・可視化するWebアプリケーションです。
...
#### GitHubのリンク  
<https://github.com/Shun42/youtube_buzz_analyzer_v2>
---
...
![一つの動画(M!LK - 爆裂愛してる (Official Music Video))の各特徴量の平均に対する大小(偏差値)](image-7.png)
...
再生数あたりの高評価率、コメント数はもちろんのこと、ポジティブワードの件数、ファン単語数などが関係しており、  
根強い好感を持っているファンが多いことが再生数が多いことと関係があることが分かります。
...
![各strong_wordsのスコアによるバズ傾向の分類](image-10.png)
...
Anacondaを使用する場合は、仮想環境を作成してからインストールします。
